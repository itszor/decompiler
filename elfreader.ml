...
let parse_ehdr elfbits =
  ...
  | { _ } ->
      raise (File_read_error "Can't parse Elf Ehdr")
...
let parse_shdr elfbits =
  ...
  | { _ } ->
      raise (File_read_error "Can't parse Elf Shdr")
...
let parse_sym elfbits =
  bitmatch elfbits with
	...
	st_shndx = st_shndx }
  | { _ } ->
      raise (File_read_error "Can't parse symbol")
  ...
  bitmatch elfbits with
    { si_boundto : 16 : littleendian;	(* Direct bindings, symbol bound to.  *)
      ...
      { si_boundto = si_boundto;
        si_flags = si_flags }
  | { _ } ->
      raise (File_read_error "Can't parse syminfo")
  ...
  bitmatch elfbits with
    { r_offset : 32 : littleendian;	(* Address.  *)
      ...
      { rel_offset = r_offset;
        rel_info = r_info }
  | { _ } ->
      raise (File_read_error "Can't parse rel")
...
let parse_rela elfbits =
  ...
  | { _ } ->
      raise (File_read_error "Can't parse rela")
...
let parse_phdr elfbits =
  bitmatch elfbits with
	...
	p_align = p_align }
  | { _ } ->
      raise (File_read_error "Can't parse phdr")
...
let parse_dyn elfbits =
  ...
  | { _ } ->
      raise (File_read_error "Can't parse dyn")
*)
...
let _ =
  let elfbits = Bitstring.bitstring_of_file "libGLESv2.so" in
