	  ...
	  (fun cfa_offset ->
	    try
		  ...
		  match var.Function.var_location with
			...
			addressable_ent.Ptrtracking.insn_addr cfa_offset
		...
		cfa_offset iaddr_str var.Function.var_name;
	      (* Mark that the variable is addressable.  *)
	      var.Function.var_addressable <- true
	    with Not_found ->
              Log.printf 3 "Can't find var for CFA offsets %ld\n" cfa_offset)
	  addressable_ent.Ptrtracking.cfa_offsets
      end)
    addressable
